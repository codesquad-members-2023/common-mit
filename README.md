...
=======
>>>>>>> a247e7806756c15946b637a79d0568defe81089c
# 용어
...
이 버전을 리셋하겠다 X 
해시번호의 버전으로 리셋하겠다 O
수정하던 것도 싹 지울때  --hard 를 붙임
수정하고 있던 것은 살리고 싶다면? --soft

- 협업시 reset은 위험함
## git revert 
삭제와 보전의 목적

git reset은 r3를 입력시 r3로 감 
하지만 git revert는 r4를 입력해야 r3로 갈 수 있다

revert시에는 단계적으로 내려가야함 
r3가 있는데 r1으로 바로 내려가지 말고 한 단계씩 내려가야한다

why? 어떤 단계 이후의 것을 모두 되돌리는 것이 아니라
해당 단계에서 일어난 변화만을 되돌리기 때문

## rebase
로그가 선형적으로 남도록 하고 싶을 때

## 
.git 디렉토리만 로컬저장소임


깃은 분산저장소라 로컬에서 로컬 클론도 가능

head는 현재 작업중인 브랜치(main)을 참조
main은 커밋(120980asdf8uew09)의 참조
